...
void on_pushButton_clicked()
{
	...
	while (cv::waitKey(1))
	{
		if (!cap.read(frame))
		{
			cout << "No frames grabbed!\n";
			break;
		}

		flip(frame, frame, 1);   // 左右镜像
		/*
		std::vector<uchar> buf;
		cvMatToJPG(frame, buf);
		*/
		string s = MatToStr(frame);
		server* servers = &m_server;
		for (auto& connection : connections) {
			servers->send(connection, s, websocketpp::frame::opcode::BINARY);//发送二进制数据
		}
	}
}
